    ...
    <div class="col-md-8">
        <nav aria-label="breadcrumb mb-5">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item">
                    <router-link :to="{name:'Show',params:{id:$route.params.id,title:$route.params.title}}">
                        {{$route.params.title}}</router-link>
                </li>
        ...
        <div>
            <div class="card">
                ...
                <div class="card-body" id="content" style="height:400px;overflow-y:scroll">
                    <div class="container col-10" v-if="success">
                        <template v-if="allMessages[0]">
                            <div class="col-5 mb-3"
                                :class="message.from.id==user.id?'ml-auto alert alert-success':'alert alert-secondary'"
                                v-for="(message,index) in messages" :key="index">
                                <img :src="message.from.avatar.encoded" alt="avatar"
                                    style="width:30px;height:30px;vertical-align:top" class="rounded-circle mr-1">
                                <strong class="card-title">
                                    {{message.from.name==user.name?"Vous":message.from.name}}</strong>
                                <p class="card-text mt-2">{{message.content}}</p>
                                <i class="float-right mt-2"><small>{{message.created_at}}</small></i>
                                <div class="clearfix"></div>
                            </div>
                        </template>
                        <div v-else-if="annoucement.user_id!=user.id">
                            <div class="alert alert-info text-center mx-auto">
                                Demarrer une conversation avec {{annoucement.user.name}}
                            </div>
                        </div>
                    </div>
                ...
                <div class="card-footer text-muted">
                    <form v-if="receiver && annoucement.user_id==user.id || annoucement.user_id!=user.id"
                        @submit.prevent="SentMessage" ref="form">
                        <div class="form-group mb-3">
                            <textarea class="form-control" placeholder="Ecrire votre message ici" v-model="content"
                                @focus="makeAsRead"></textarea>
                    ...
                    <div class="row" v-if="list[0]">
                        <div v-for="(item,index) in list" :key="index" class="col-2 text-center">
                            <a href="" @click.prevent="selectUser(item.id)" style="position:relative;color:white">
                                <img :src="item.avatar.encoded" alt="avatar" class="rounded-circle"
                                    :class="unreadNotifications.some(i=>i.data.message.from.id==item.id)?'unread':''"
                                    style="width:100%">
                                <span v-if="unreadNotifications.some(i=>i.data.message.from.id==item.id)"
                                    class="badge bg-danger"
                                    style="position:absolute;top:-8px">{{unreadNotifications.filter(i=>i.data.message.from.id==item.id).length}}</span>
                            </a>
...
<script>
    ...
    export default {
                ...
                axios.get(`/api/getMessages/${this.$route.params.id}/${this.user.id}/${prop}`, {
                    ...
                    if (res.data.list) {
                        ...
                        let data = res.data.list.map(item => Object.values(item).filter(i => i.id != this.user
                            .id)[
                        0]); //on a maintenant dans la variable data la liste des contacts avec des contacts dupliqués
                        this.list = Array.from(new Set(data.map(a => a.id))).map(
                        id => { //pour enlever les contacts dupliqués
        ...
        watch: {
            annoucement() {
                this.getMessages(); 
                //demander les messages une fois l'annonce est recupérée.
            },
        },
        mounted() {
            ...
            window.Echo.private('App.Models.User.' + this.user.id).notification((data) => {
                ...
                if (this.annoucement.user_id == this.user
                    .id) { // si l'utilisateur authentifié est l'annonceur if faut updater la list des contact .
                    let listOfId = this.list.map(i => i.id);
                    if (listOfId.includes(data.message.from
                        .id)) { //ne mettre à jour la liste des contacts que s'il s'agit d'un contact qui n'exist pas encore.
                        this.allMessages.push(data.message)
                    } else {
                        this
                    .getMessages(); //pour modifier la liste des contacts il faut demander à nouveau la liste des messages est executer les filtres javascript.
                    }
                }
                this.allMessages.push(data
                    .message) //si non il suffit d'ajouter le nouveau message au liste des messages.
            });
        },
        updated() {
            document.getElementById("content").scrollTo(0, document.getElementById("content").scrollHeight);
